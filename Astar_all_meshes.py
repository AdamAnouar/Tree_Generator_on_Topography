...
#Playground is more suited for orthogonal networks (XY grid)
...
def Playground_alternative(srt_pts, end_pts, sorted_pts):
    ...
    for srt_pt,end_pt in zip(srt_pts, end_pts):
        if ((abs(srt_pt.Z) - abs(end_pt.Z)) / math.sqrt((12**2) + (abs(srt_pt.Z) - abs(end_pt.Z))**2)) < math.sin(max_Angle) and srt_pt not in excluded_pts and end_pt not in excluded_pts:
